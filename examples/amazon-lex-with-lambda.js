...
/**
 * Lex appointment blueprint example
 */

//---------------------------------------- Get Instance ---------------------------
const Botanalytics = require("botanalytics").AmazonLex("<token>", { debug:true } );
...
exports.handler = (event, context, callback) => {
    try {
        ...
        console.log('\nfunctionName = %s \nAWSrequestID = %s \nlogGroupName =%s \nlogStreamName =%s \nclientContext =%s \ntimeout=%s',
            ...
            JSON.stringify(context.clientContext),
...
//-----------------------------END of Logging----------------------------
            loggingCallback(response, callback);
        });
    } catch (err) {
        callback(err);
    }
